...
- **Every API route and every page must check auth.** Route handlers under `app/api/**` call `getServerSession(authOptions)` from `lib/auth.ts` and return `401` when there's no session; pages call `getServerSession(authOptions)` (server) or `useSession()` (client) and redirect to `/api/auth/signin`. Always pass `authOptions` — a bare `getServerSession()` skips the `session`/`jwt` callbacks and `session.user.role` comes back `undefined`. Don't add a new route or page that skips the auth check — see `CHANGELOG.md`'s Unreleased entry for why it matters (these routes had no auth check until 2026-09-07).
- **Two roles, enforced server-side.** `editor` (`ALLOWED_EMAILS`) has full read/write; `viewer` (`READONLY_EMAILS`) can read but not write. Use `canEdit(session.user?.role)` from `lib/auth.ts` to gate mutations. Every mutating route (`POST`/`PATCH`) must check `canEdit` and return `403` for viewers — hiding the button in the UI is a courtesy, not the enforcement.
